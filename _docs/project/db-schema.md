...
> **Note**: In the older schema, we had `grid_size` and `winning_types` in `games`. Now, those can be handled by **Card Types** or **Sessions**, as needed.
...
| `is_mandatory`| boolean   | If `true`, item must appear on all cards referencing this list (or see pivot logic for Card Types). |
| `created_at`  | timestamp | Defaults to `now()`.                                                                            |
| `updated_at`  | timestamp | Defaults to `now()`, auto-updates.                                                              |
...
When the game goes “live,” a **Session** is created. This can reference one or more **Card Types**, or revolve around a single type.
...
*(If multiple rounds are needed, you can add a separate `rounds` table referencing `sessions(id)` + `card_type_id` for each round.)*
...
| `item_id`    | uuid       | References `list_items(id)` (or older `game_items(id)` if used).           |
...
We use Next.js Server Actions to handle DB queries.  
Real-time calls can use frequent polling or websockets in the future.  
We keep concurrency low by letting only certain roles call items.

---

## 9. Future Enhancements

- **account_members** for more advanced roles.
...
- **Accounts / Profiles** for multi-user teams.
- **Game** as the top-level container.
- **Lists / List Items** for item sets. 
- **Card Types** referencing a List for layout/styling.
- **Cards** generated from a Card Type.
- **Sessions** for real-time play with **Calls** and **Claims** tracking.
